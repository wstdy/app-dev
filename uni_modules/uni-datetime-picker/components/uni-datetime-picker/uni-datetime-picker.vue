	...
	<view class="uni-date">
		<view class="uni-date-editor" @click="show">
			...
			'uni-date-x--border': border && isRange}">
					<view v-if="!isRange" class="uni-date-x uni-date-single x-date__x-input">
						<!-- <uni-icons type="calendar" color="#8d97a3" size="22"></uni-icons> -->
						<view class="uni-date__x-input" :class="{'t-color':!singleVal}">
							<text class="x-date-text" :class="{'t-color':!singleVal}">{{singleVal||singlePlaceholderText}}</text>
						</view>
					</view>
					<view v-else class="uni-date-x uni-date-range">
						<uni-icons type="calendar" color="#8d97a3" size="22"></uni-icons>
						<view class="uni-date__x-input " :class="{'t-color':!range.startDate}"
							 ><view class="x-text">{{range.startDate||startPlaceholderText}}</view></view>
						<slot><view class="">{{rangeSeparator}}</view> </slot>
						<view class="uni-date__x-input " :class="{'t-color':!range.endDate}"
							 ><view class="x-text">{{range.endDate||endPlaceholderText}}</view></view>
					</view>
					<view v-if="!isRange && !disabled" class="uni-date__icon-clear x-date-icon">
						<uni-icons type="right" color="#8d97a3" size="20"></uni-icons>
					</view>
					<view v-else-if="showClearIcon" class="uni-date__icon-clear" @click.stop="clear">
	...
	export default {
		...
		watch: {
			...
			startTime(newVal) {
				...
				const {
					...
					defDate,
					defTime
				} = this.parseDate(new Date(newVal))
				// this.caleRange.startDate = defDate
				// if (this.hasTime) {
				// 	this.caleRange.startTime = defTime
				// }

				// this.range.startDate = this.formatType == 'year' ? newVal : this.formatType == 'month' ? defMonth : defDate;
				// if (this.hasTime) {
				// 	this.range.startDate = this.range.startDate + ' ' + defTime
				// }
				// const displayRange = [this.range.startDate, this.range.endDate]
			...
			endTime(newVal) {
				...
				// 	this.caleRange.endTime = defTime
				// }
				// this.range.endDate = this.formatType == 'year' ? newVal : this.formatType == 'month' ? defMonth : defDate
				// if (this.hasTime) {
				// 	this.range.endDate = this.range.endDate + ' ' + defTime
				// }
			},
		},
		computed: {
		...
		mounted() {
			...
			if (this.startTime && this.endTime) {
				this.initPicker([this.startTime, this.endTime])
			} else if (!this.isRange) {
				this.initPicker(this.modelValue || this.value)
			}
		...
		methods: {
			initPicker(newVal) {
				...
				if (!Array.isArray(newVal) && !this.isRange) {
				  ...
				  this.singleVal = this.formatType == 'year' ? defYear : this.formatType == 'month' ? defMonth : defDate
					...
					this.range.startDate = this.formatType == 'year' ? before : this.formatType == 'month' ? defBefore.defMonth :
						startDate
					this.range.endDate = this.formatType == 'year' ? after : this.formatType == 'month' ? defAfter.defMonth :
						endDate

					if (this.hasTime) {
						this.range.startDate = this.range.startDate + ' ' + defBefore.defTime
						this.range.endDate = this.range.endDate + ' ' + defAfter.defTime
			...
			setEmit(value) {
				...
				//如果是数组 代表是 范围 默认emit startTime  endTime
				if (Array.isArray(value)) {
			...
			mobileChange(e) {
				if (this.isRange) {
					...
					if (this.hasTime) {
						...
						this.tempRange.startTime = startTime || '00:00:00'
						this.tempRange.endTime = endTime || '00:00:00'
					}
			...
			handleStartAndEnd(before, after, temp = false) {
				...
				let defBefore = this.parseDate(before);
				let defAfter = this.parseDate(after)
				let start = this.formatType == 'year' ? defBefore.defYear : this.formatType == 'month' ? defBefore.defMonth :
					before;
				let end = this.formatType == 'year' ? defAfter.defYear : this.formatType == 'month' ? defAfter.defMonth : after;
				if (this.dateCompare(before, after)) {
					this[type].startDate = start
					this[type].endDate = end
				} else {
					this[type].startDate = end
					this[type].endDate = start
				}
			},
			...
			parseDate(date) {
				...
				return {
					defYear: year,
					defMonth: year + '-' + this.lessTen(month),
					defDate,
					defTime
				}
			},
