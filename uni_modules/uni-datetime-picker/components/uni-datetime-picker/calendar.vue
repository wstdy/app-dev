	...
	export default {
		...
		computed: {
			reactStartTime() {
				...
				const res = activeDate === this.startDate ? this.selectableTimes.start :(this.range ?'00:00:00':'')
				this.timeRange.startTime=res
				return res
			},
			reactEndTime() {
				const activeDate = this.range ? this.tempRange.after : this.calendar.fullDate
				const res = activeDate === this.endDate ? this.selectableTimes.end : (this.range ?'00:00:00':'')
				this.timeRange.endTime=res
				return res
			},
			/**
			 * for i18n
		...
		methods: {
			...
			clearCalender() {
				if (this.range) {
					...
					this.tempSingleDate = this.cale.getDate(new Date()).fullDate
				}
				//this.calendar.fullDate = ''
				this.setDate()
			},
