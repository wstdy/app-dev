...
<script>
	export default {
		...
		created() {
			...
			// 	this.rgba=this.hexToRgba(this.color,1)
			// }else{
			// 	let arr=this.color.split('(')[1].split(')')[0].split(',')

			// 	arr.forEach((it,i)=>{
			// 		if(i==0) this.rgba.r=it;
			// 		if(i==1) this.rgba.g=it;
			// 		if(i==2) this.rgba.b=it;
			// 		if(i==3) this.rgba.a=it;
			// 	})
			// }
		...
		methods: {
			...
			confirm() {
				if(this.mode){
					this.$emit('confirmData', this.hex)
				}else{
					let str="rgba("+this.rgba.r+','+this.rgba.g+','+this.rgba.b+','+this.rgba.a+")"
					this.$emit('confirmData', str)
				}
			...
			*/
			hexToRgba(hex, opacity) {
				return {r:parseInt("0x" + hex.slice(1, 3)),g:parseInt("0x" + hex.slice(3, 5)),b: parseInt("0x" + hex.slice(5, 7)),a:opacity};
			},
			/**
			 * 设置 rgb 颜色
			 */
