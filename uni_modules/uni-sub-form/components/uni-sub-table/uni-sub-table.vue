  ...
  <uni-popup ref="tableRef" type="share" :is-mask-click="false" safeArea backgroundColor="#fff" class="popup-box">
    <view class="model-box">
      <view class="model-head">
        ...
        <text class="text-title">选择数据</text>
      </view>
      <view class="model-content">
        <Search class="search-box" @search="search" />
				...
				<view v-show="activeRef === 0">
					...
					<view class="page-box">
						<uni-pagination :current="curpage+1" :total="total" :show-icon="true" @change="pageChange" />
					</view>
				</view>
				<view v-show="activeRef === 1">
					<uni-table class="table-box" border stripe  emptyText="暂无更多数据" >
						<!-- 表头行 -->
						<uni-tr>
							<uni-th v-for="(item,index) in tableTh" :key="index" align="center">{{item.title}}</uni-th>
						</uni-tr>
						<!-- 表格数据行 -->
						<uni-tr v-for="(item,index) in selectedList" :key="index">
							<uni-td align="center" v-for="(title,idx) in tableTh">
								{{item[title.name]}}
							</uni-td>
						</uni-tr>
...
<script setup>
  ...
  const selectedList = ref([])
	const tempselectedList = ref([new Array(10)])
  ...
  function open() {
    tableRef.value.open();
    tableTh.value = props.associateOptions.map(item => {
      return {
        name: item.name,
  ...
  async function getData() {
    if (props.itemId) {
      ...
      let res = await changeApiOptions(props.apiConfig);
      if (res.data) {
        dataSource.value = Array.isArray(res.data) ? res.data : []
      }
    } 
  ...
  function pageChange(e) {  
		curpage.value=e.current - 1;
		if(!tempselectedList.value[curpage.value]){ 
			tempselectedList.value[curpage.value]=new Array(10)
		} 
    ...
    showDataSource.value = searchTxt.value ? searchDataSource.value.slice(startIndex, startIndex + 10) : dataSource
      .value.slice(startIndex, startIndex + 10)
		...
		let index=[]
		tempselectedList.value[curpage.value].forEach((o,i)=>{ 
			if(o){
				index.push(i)
			}
		})
		tableInfoRef.value.clearSelection()
		 tableInfoRef.value.toggleRowSelection(index,true)
  }


  function selectionChange(e) {
    const indexArr = e.detail.index 
    showDataSource.value.find((item, index) => {
			let obj=''
			if(indexArr.includes(index)){obj=item}
			tempselectedList.value[curpage.value][index]=obj
		});
		selectedList.value=[]
		tempselectedList.value.forEach(arr=>{
			arr.forEach(o=>{
				if(o) selectedList.value.push(o)
			})
